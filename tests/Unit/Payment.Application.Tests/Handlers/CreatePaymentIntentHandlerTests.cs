...
public class CreatePaymentIntentHandlerTests
{
    ...
    public async Task Handle_ValidCommand_ShouldCreateAndCaptureIntent()
    {
        ...
        _gatewayMock.Setup(g => g.AuthorizeAsync(command.MerchantId, It.IsAny<Money>(), It.IsAny<CardDetails?>(), It.IsAny<CardSecurityCode?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()))
    ...
    public async Task Handle_ValidCardToken_ShouldResolveCardFromVault()
    {
        ...
        SetupMerchantConfig(autoCapture: true);
        _gatewayMock.Setup(g => g.AuthorizeAsync(merchantId, It.IsAny<Money>(), It.IsAny<CardDetails?>(), It.IsAny<CardSecurityCode?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<GatewayResponse>.Success(new GatewayResponse(true, "AUTH123", "GW-1", null)));
        _uowMock.Setup(u => u.SaveChangesAsync(It.IsAny<CancellationToken>())).ReturnsAsync(1);
        ...
        Assert.True(result.IsSuccess);
        _gatewayMock.Verify(g => g.AuthorizeAsync(merchantId, It.IsAny<Money>(), It.Is<CardDetails>(c => c.LastFour == "4242" && c.Brand == "Visa" && c.Token == "card_abc123"), It.IsAny<CardSecurityCode?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
