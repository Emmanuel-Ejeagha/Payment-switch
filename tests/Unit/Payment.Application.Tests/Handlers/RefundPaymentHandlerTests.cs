...
public class RefundPaymentHandlerTests
{
    ...
    public async Task Handle_CapturedIntent_ShouldRefund()
    {
        ...
        _gatewayMock.Setup(g => g.RefundAsync(intent.MerchantId, intent.GatewayReference!, It.IsAny<Money>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()))
    ...
    public async Task Handle_NonCapturedIntent_ShouldFailWithInvalidTransition()
    {
        ...
        _gatewayMock.Verify(g => g.RefundAsync(It.IsAny<Guid>(), It.IsAny<GatewayReference>(), It.IsAny<Money>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.Never);
    }
