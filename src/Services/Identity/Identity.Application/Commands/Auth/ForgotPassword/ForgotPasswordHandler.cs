...
namespace Identity.Application.Commands.Auth.ForgotPassword;
...
public class ForgotPasswordHandler
{
    ...
    {
        _logger.LogInformation("Handling {CommandName} for {Identifier}", nameof(ForgotPasswordCommand), DataMasker.MaskEmail(command.Email));
        ...
        if (user == null)
        {
            ...
            _logger.LogWarning("Forgot-password requested for unknown {Identifier}", DataMasker.MaskEmail(command.Email));
        ...
        var message = ResetPasswordEmailBuilder.Build(user.Email.Value, token.PlainText, _options.Subject, _options.FrontendBaseUrl);
        var sendResult = await _emailSender.SendAsync(message, cancellationToken);
        if (sendResult.IsFailure)
            _logger.LogError("Failed to send password-reset email to {Identifier}: {Errors}",
