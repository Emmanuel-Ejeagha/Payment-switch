...
public class ReplayWebhookEventHandler
{
    ...
    public async Task<Result<WebhookEventDto>> Handle(ReplayWebhookEventCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (!ownerResult.IsSuccess || !command.Caller.CanAccess(ownerResult.Value))
        ...
        _logger.LogWarning(
            "Webhook {EventId} for Merchant {MerchantId} replayed by {CallerEmail} ({CallerId})",
            command.EventId, command.MerchantId, command.Caller.Email, command.Caller.UserId);
